...
vector<int> query(int x1, int x2, int y1, int y2) {
  ...
  while (!ehfolha(pos)) {
    if (l <= tree2d[pos].xmid) {
      ...
      vector<int> toadd = tree2d[2 * pos + 2].assoc.query(0, y1, y2);
      /*   cout << "going left:\n";
  ...
  if (l <= tree2d[pos].xmid && tree2d[pos].xmid <= r) {
    vector<int> toadd = tree2d[pos].assoc.query(0, y1, y2);
    answer.insert(answer.end(), all(toadd));
  }
  ...
  while (!ehfolha(pos)) {
    if (r >= tree2d[pos].xmid) {
      ...
      vector<int> toadd = tree2d[2 * pos + 1].assoc.query(0, y1, y2);
      answer.insert(answer.end(), all(toadd));
      /* cout << "going right:\n";
...
int32_t main() {
  ...
  while (q--) {
    ...
    vector<int> answer = query(x1, x2, y1, y2);
    bool first = true;
    sort(all(answer));
    for (auto &id : answer) {
      if (!first) cout << " ";
