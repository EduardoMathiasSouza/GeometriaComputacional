...
struct item{
    ...
    item(int _l, int _r, bool _closed_left, bool _closed_right) : l{_l}, r{_r}, closed_left{_closed_left}, closed_right{_closed_right}{
    }    

    bool closed(){
        return closed_left && closed_right;
    }
    bool half(){
        return (!closed_left) && (!closed_right);
    }
...
struct node{
    ...
    node operator+(node oth){
        node c;
        c.limites = item(this->limites.l, oth.limites.r, 
                        this->limites.closed_left, oth.limites.closed_right);
        return c;
    }
...
vector<ii> query(int qx, int pos, int lx, int rx){
    ...
    // qx \in Int(lc(v)); Como fazer isso em O(1)?
